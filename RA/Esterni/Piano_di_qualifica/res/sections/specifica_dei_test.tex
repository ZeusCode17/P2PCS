...
\end{itemize}
	Nella seguente tabella vengono anche tracciati i requisiti a cui fanno riferimento. Tali requisiti sono indicati nel documento \textit{Analisi dei requisiti v4.0.0}, sezione §4.
	...
	%	& \textbf{\color{white}Fonti} 
		...
		TA2	& RFD1 &	L'utente deve poter visualizzare la guida introduttiva. All'utente viene chiesto di:
		 ...
		 \end{itemize}	\tabularnewline
		 TA6.2 & RFO5.2 \\ RFO5.2.1 \\ RFO5.2.2 \\ RFO5.2.3 &	L'utente autenticato deve poter visualizzare i dettagli del veicolo e che siano disponibili i bottoni per visualizzare le disponibilità, le statistiche e la rimozione del veicolo. All'utente viene chiesto di:
		 \begin{itemize}
		 	...
		 	\end{itemize}
		 \end{itemize}		\tabularnewline
		 TA7 & RFO6 \\ RFO6.1  & L'utente autenticato deve poter visualizzare la lista di tutte le sue prenotazioni attive.
		 ...
		 \end{itemize}	\tabularnewline
		 TA7.1 & RFO6.2 & L'utente proprietario del veicolo può confermare o annullare una richiesta di prenotazione. All'utente è richiesto di:
		 \begin{itemize}
		 ...
		 \end{itemize}		\tabularnewline
	 	 TA7.2 & RFO6.3 & L'utente proprietario del veicolo può confermare l'avvenuta consegna delle chiavi del veicolo all'usufruente. All'utente è richiesto di:
	 	 \begin{itemize}
	 	 	\item accedere alla schermata di visualizzazione delle prenotazioni attive;
	 	 ...
	 	 \end{itemize}		\tabularnewline
 	 	 TA7.3 & RFO6.4 & L'utente proprietario del veicolo può chiudere la prenotazione nel momento in cui ha ricevuto il veicolo e le chiavi sbloccando così la possibilità di recensire l'utente usufruente. All'utente è richiesto di:
 	 	 \begin{itemize}
 	 	 	\item accedere alla schermata di visualizzazione delle prenotazioni attive;
 	 	 ...
 	 	 \end{itemize}	\tabularnewline
		 TA7.4 & RFO6.5 &	L'utente usufruente deve poter annullare una prenotazione in attesa di conferma da parte del proprietario. All'utente viene chiesto di:
		 \begin{itemize}
		 	\item accedere alla schermata di visualizzazione delle prenotazioni attive;
		 ...
		 \end{itemize}		\tabularnewline
	 	 TA7.5 & RFO6.6 & L'utente usufruente deve poter chiudere la prenotazione nel momento in cui riconsegni il veicolo e le chiavi sbloccando così la possibilità di recensire l'utente proprietario del veicolo. All'utente viene chiesto di:
	 	 \begin{itemize}
	 		\item accedere alla schermata di visualizzazione delle prenotazioni attive;
	 		\item verificare che la prenotazione attiva sia in corso;
	 		\item selezionare la prenotazione;
	 		...
	 		\item verificare che l'utente si ritrovi nella schermata di visualizzazione delle prenotazioni attive e che la prenotazione non sia presente nella lista di prenotazioni attive.
	 	 \end{itemize}		\tabularnewline 				
		 TA8	& RFO7 \\ RFD7.1 \\ RFD7.2 \\ RFD7.3 \\ RFD7.4 \\ RFO7.5 &	L'utente autenticato deve poter effettuare una ricerca specifica sui veicoli disponibili e poter proseguire con la prenotazione di uno di essi. All'utente viene chiesto di:
		 \begin{itemize}
		 ...
		 \end{itemize}		\tabularnewline
		 TA10.1.2	& RFO9.2.8 &	L'utente autenticato deve poter aggiornare la password del proprio account. All'utente viene chiesto di:
		 \begin{itemize}
		 ...
		 \end{itemize}		\tabularnewline
		 TA10.2 & RFO9.3 	&	L'utente autenticato deve poter cancellare il proprio profilo e i relativi dati. All'utente viene chiesto di:
		 \begin{itemize}
		 ...
		 \end{itemize}		\tabularnewline
		 TA11	& RFF10 &	L'utente autenticato deve poter accedere alla pagina \textit{Gioca} dell'applicazione. All'utente e richiesto di:
		 \begin{itemize}
		 ...
		 \end{itemize}		\tabularnewline
		 TA11.1	& RFF10.1 &	L'utente autenticato deve visualizzare la tabella Milestone Unlock\glo, che illustra i premi ottenibili dal raggiungimento di determinati livelli d'esperienza. All'utente autenticato viene chiesto di:
		 \begin{itemize}
		 ...
		 \end{itemize}		\tabularnewline		 
		 TA11.1.1	& RFF10.1.1 \\ RFF10.1.2 &	L'utente autenticato che ha raggiunto un nuovo livello esperienza, superiore al 5, deve ricevere il rispettivo premio illustrato nella tabella Milestone Unlock\glo. All'utente autenticato che raggiunto un nuovo livello superiore al quinto viene chiesto di:
		 \begin{itemize}
		 	\item verificare che l'applicazione abbia consegnato il relativo premio illustrato nella tabella Milestone Unlock.
		 \end{itemize}		\tabularnewline
		 TA11.2 & RFF10.2 &	L'utente autenticato deve visualizzare la ruota della fortuna Lucky Spin\glo. All'utente autenticato viene chiesto di:
		 \begin{itemize}
		 	\item verificare che il bottone presente nella pagina \textit{Gioca} sia disponibile ad essere premuto;
		 	\item verificare che si possa visualizzare la Lucky Spin.
		 \end{itemize}		\tabularnewline		 
		 TA11.2.1	& RFF10.2.1 \\ RFF10.2.2 &	L'utente autenticato che ha appena concluso una prenotazione deve ricevere un tentativo per la Lucky Spin\glosp che può riprendere anche in un secondo momento. All'utente autenticato che ha concluso una prenotazione viene chiesto di:
		 \begin{itemize}
		 ...
		 \end{itemize}		\tabularnewline	
		 TA11.3	& RFF10.3 &	L'utente generico deve poter visualizzare la classifica degli utenti migliori. All'utente viene chiesto di:
		 \begin{itemize}
		 	\item verificare che il bottone presente nella pagina \textit{Gioca} sia disponibile ad essere premuto;
		 	\item verificare che le classifiche siano visualizzate correttamente.
		 \end{itemize}		\tabularnewline	
		 TA11.3.1	& RFF10.3.1 & Dopo un tempo prestabilito, i primi tre utente della classifica devono ricevere un premio. All'utente che si è classificato tra le prime tre posizioni, nella data prestabilita, viene chiesto di:
		 \begin{itemize}
		 	\item verificare di aver ricevuto il premio promesso.
		 \end{itemize}		\tabularnewline	
	 	 TA11.4	& RFF10.4	& L'utente autenticato deve poter visualizzare la tabella Daily Rewards che illustra i premi giornalieri del mese corrente. All'utente viene chiesto di :
	 	 \begin{itemize}
	 	 	\item verificare che il bottone presente nella pagina \textit{Gioca} sia disponibile ad essere premuto;
	 	 	\item verificare che si possa accedere e visualizzare la tabella dei Daily Rewards.
	 	 \end{itemize}		\tabularnewline	
	 	 TA11.4.1	& RFF10.4.1 \\ RFF10.4.1.1 &	L'utente autenticato deve poter ritirare il premio del giorno corrente dalla tabella Daily Rewards. \\All'utente viene chiesto di:
	 	 \begin{itemize}
	 	 ...
	 	 \end{itemize}		\tabularnewline
		 TA12	& RFF11 &	L'utente autenticato deve avere un codice personale visualizzabile nella schermata di gestione del profilo, da inviare ad amici che non sono ancora registrati al servizio offerto da \textit{GaiaGo}.  All'utente autenticato viene chiesto di:
		 \begin{itemize}
		 	\item accedere alla sezione \textit{Gestione Profilo};
		 	\item verificare che sia presente il suo codice personale.
		 \end{itemize}		\tabularnewline
		 TA13	& RFF2.6 & 	L'utente che si registra inserendo un codice amico deve ricevere un premio dopo la registrazione. Tale premio deve essere corrisposto anche al proprietario del codice. All'utente che si è registrato inserendo un codice amico viene chiesto di:
		 \begin{itemize}
		 ...
		 \end{itemize}		\tabularnewline	
	 	 TA14	& RFF12 &	L'utente autenticato deve poter accedere al Minigioco che consiste in un garage che dà la possibilità di effettuare modifiche ad un'auto. All'utente viene chiesto di:
		 \begin{itemize}
		 ...
		 \end{itemize}		\tabularnewline
		 TA14.1	& RFF12.1  &	L'utente autenticato deve poter accedere al Minigioco del garage in cui potrà modificare le prestazioni dell'auto base. All'utente viene chiesto di:
		 \begin{itemize}
		 ...
		 \end{itemize}		\tabularnewline
		 TA14.2	& RFF12.2	& L'utente autenticato deve poter accedere al Minigioco del garage in cui potrà modificare l'estetica dell'auto base. All'utente viene chiesto di:
		 \begin{itemize}
...
\end{itemize}
	...
	& \textbf{\color{white}Stato} 
		...
		TS1	& RFO2 & Verificare che il sistema permetta all'utente di effettuare la registrazione all'applicazione qualora non lo fosse. & N.I.	\tabularnewline
		 ...
		 TS6	& RFO5 & Verificare che il sistema permetta all'utente autenticato di poter gestire e visualizzare i propri veicoli. 
		 \tabularnewline
		 TS6.1 & RFO5.1 &	Verificare che il sistema permetta all'utente autenticato di poter inserire un nuovo veicolo. \tabularnewline
		 TS6.2 & RFO5.2 \\ RFO5.2.1 \\ RFO5.2.2 \\ RFO5.2.3 &	Verificare che il sistema permetta all'utente autenticato di poter visualizzare i dettagli del veicolo e di rendere disponibili i bottoni per visualizzare le disponibilità, le statistiche e la rimozione del veicolo. \tabularnewline
		 TS7 & RFO6 \\ RFO6.1  & Verificare che il sistema permetta all'utente autenticato di poter visualizzare la lista di tutte le sue prenotazioni attive.
		 Per ogni prenotazione attiva presente nella lista l'utente deve poter visualizzare i dettagli riassuntivi (marca, modello, stato della prenotazione, data, fascia oraria e foto del veicolo) ed eseguire operazioni di gestione su di esse (visualizzazione dettaglio). \tabularnewline
		 TS7.1 & RFO6.2 & Verificare che il sistema permetta all'utente autenticato come proprietario del veicolo di confermare o annullare una richiesta di prenotazione. \tabularnewline
		 TS7.2 & RFO6.3 & Verificare che il sistema permetta all'utente autenticato come proprietario del veicolo di confermare l'avvenuta consegna delle chiavi del veicolo all'usufruente. \tabularnewline
		 TS7.3 & RFO6.4 & Verificare che il sistema permetta all'utente autenticato come proprietario del veicolo di chiudere la prenotazione nel momento in cui ha ricevuto il veicolo e le chiavi sbloccando così la possibilità di recensire l'utente usufruente. \tabularnewline
		 TS7.4 & RFO6.5 &	Verificare che il sistema permetta all'utente autenticato come usufruente di poter annullare una prenotazione in attesa di conferma da parte del proprietario. \tabularnewline
		 TS7.5 & RFO6.6 & Verificare che il sistema permetta all'utente autenticato come usufruente di poter chiudere la prenotazione nel momento in cui riconsegni il veicolo e le chiavi sbloccando così la possibilità di recensire l'utente proprietario del veicolo. 		\tabularnewline 				
		 TS8	& RFO7 \\ RFD7.1 \\ RFD7.2 \\ RFD7.3 \\ RFD7.4 \\ RFO7.5 &	Verificare che il sistema permetta all'utente autenticato di poter effettuare una ricerca specifica sui veicoli disponibili e poter proseguire con la prenotazione di uno di essi. \tabularnewline
		 TS9 &	RFD8 & Verificare che il sistema permetta all'utente autenticato di poter visualizzare una lista contente tutte le sue prenotazioni concluse. \tabularnewline
		 TS10	& RFO9 & Verificare che il sistema permetta all'utente autenticato di poter visualizzare, modificare e cancellare il proprio account con i relativi dati. \tabularnewline
		 TS10.1 & RFO9.2 & Verificare che il sistema permetta all'utente autenticato di poter aggiornare i dati del proprio account. \tabularnewline
		 TS10.1.1	& RFO9.1 & Verificare che il sistema permetta all'utente autenticato di poter aggiornare la patente di guida del proprio account. \tabularnewline
		 TS10.1.2	& RFO9.2.8 &	Verificare che il sistema permetta all'utente autenticato di poter aggiornare la password del proprio account. \tabularnewline
		 TS10.2 & RFO9.3 	&	Verificare che il sistema permetta all'utente autenticato di poter cancellare il proprio profilo e i relativi dati. \tabularnewline
		 TS11	& RFF10 &	Verificare che il sistema permetta all'utente autenticato di poter accedere alla pagina \textit{Gioca} dell'applicazione. \tabularnewline
		 TS11.1	& RFF10.1 &	Verificare che il sistema permetta all'utente autenticato di visualizzare la tabella Milestone Unlock\glo, che illustra i premi ottenibili dal raggiungimento di determinati livelli d'esperienza. 		\tabularnewline		 
		 TS11.1.1	& RFF10.1.1 \\ RFF10.1.2 &	Verificare che il sistema permetta all'utente autenticato che ha raggiunto un nuovo livello esperienza, superiore al 5, di ricevere il rispettivo premio illustrato nella tabella Milestone Unlock\glo. \tabularnewline
		 TS11.2 & RFF10.2 &	Verificare che il sistema permetta all'utente autenticato di visualizzare la ruota della fortuna Lucky Spin\glo. 		\tabularnewline		 
		 TS11.2.1	& RFF10.2.1 \\ RFF10.2.2 &	Verificare che il sistema permetta all'utente autenticato che ha appena concluso una prenotazione di ricevere un tentativo per la Lucky Spin\glosp che può riprendere anche in un secondo momento. \tabularnewline	
		 TS11.3	& RFF10.3 &	Verificare che il sistema permetta all'utente autenticato di poter visualizzare la classifica degli utenti migliori.\tabularnewline	
		 TS11.3.1	& RFF10.3.1 & Verificare che il sistema, dopo un tempo prestabilito, permetta ai primi tre utenti della classifica di ricevere un premio. 	\tabularnewline	
		 TS11.4	& RFF10.4	& Verificare che il sistema permetta all'utente autenticato di poter visualizzare la tabella Daily Rewards che illustra i premi giornalieri del mese corrente. \tabularnewline	
		 TS11.4.1	& RFF10.4.1 \\ RFF10.4.1.1 &	Verificare che il sistema permetta all'utente autenticato di poter ritirare il premio del giorno corrente dalla tabella Daily Rewards.	\tabularnewline
		 TS12	& RFF11 &	Verificare che il sistema permetta all'utente autenticato di avere un codice personale visualizzabile nella schermata di gestione del profilo, da inviare ad amici che non sono ancora registrati al servizio offerto da \textit{GaiaGo}.	\tabularnewline
		 TS13	& RFF2.6 & 	Verificare che il sistema permetta all'utente che si registra inserendo un codice amico di ricevere un premio dopo la registrazione. Tale premio deve essere corrisposto anche al proprietario del codice. \tabularnewline	
		 TS14	& RFF12 &	Verificare che il sistema permetta all'utente autenticato di poter accedere al Minigioco che consiste in un garage che dà la possibilità di effettuare modifiche ad un'auto. 	\tabularnewline
		 TS14.1	& RFF12.1  &	Verificare che il sistema permetta all'utente autenticato di poter accedere al Minigioco del garage in cui potrà modificare le prestazioni dell'auto base.	\tabularnewline
		 TS14.2	& RFF12.2	& Verificare che il sistema permetta all'utente autenticato di poter accedere al Minigioco del garage in cui potrà modificare l'estetica dell'auto base.	\tabularnewline
\end{longtable}
